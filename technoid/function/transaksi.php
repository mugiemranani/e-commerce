...
function bayar($post)
{
    ...
    if ($img['error'] == 4) {
        ...
        return header('location:' . url . 'user/bayar.php');
...
function batalkanTransaksi($id)
{
    ...
    $query = mysqli_query($konek, "SELECT * FROM transaksi WHERE id_pesan='$id_pesan'");
    $transaksi = mysqli_fetch_object($query);

    // Hapus pembayaran terkait jika ada
    mysqli_query($konek, "DELETE FROM pembayaran WHERE id_pesan='$id_pesan'");

    // Hapus detail transaksi
    mysqli_query($konek, "DELETE FROM transaksi_detail WHERE id_pesan='$id_pesan'");

    // Kembalikan stok produk
    $query_detail = mysqli_query($konek, "SELECT * FROM transaksi_detail WHERE id_pesan='$id_pesan'");
    while ($detail = mysqli_fetch_object($query_detail)) {
        $id_produk = $detail->id_produk;
        $kuantiti = $detail->kuantiti;

        // Update stok produk
        $query_produk = mysqli_query($konek, "SELECT * FROM produk WHERE id_produk='$id_produk'");
        $produk = mysqli_fetch_object($query_produk);
        $stok_baru = $produk->stok + $kuantiti;
        mysqli_query($konek, "UPDATE produk SET stok='$stok_baru' WHERE id_produk='$id_produk'");
    }

    // Hapus transaksi utama
    mysqli_query($konek, "DELETE FROM transaksi WHERE id_pesan='$id_pesan'");

    // Redirect kembali ke halaman profil atau halaman lain yang sesuai
    $_SESSION['pesan'] = "Transaksi dengan ID Pesan $id_pesan berhasil dibatalkan.";
    return header('location:' . url . 'user/bayar.php');
}
